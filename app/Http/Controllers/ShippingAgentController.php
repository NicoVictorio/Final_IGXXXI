...
class ShippingAgentController extends Controller
{
    ...
    public function getTierSAExport(Request $request)
    {
        ...
        $cekBayangan = ShippingContainer::select('isa_due_date')->where('isa_due_date', $bay)->where('ica_target_row', $row)->where('ica_sequence', $cekNomor)->where('Team_id', $idTeam)->where('Period_id', 1)->count();
        if($cekBayangan == 1){
            $cekNomor += 2;
        }
